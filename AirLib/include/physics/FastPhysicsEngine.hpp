...
private:
    ...
    static Wrench getWindWrench(const PhysicsBody& body, const Quaternionr& orientation, const Vector3r& wind_world)
    {
        // Add forces due to wind
        // https://physics.stackexchange.com/questions/28827/does-wind-speed-things-up-or-accelerate-things
        // https://www.engineeringtoolbox.com/wind-load-d_1775.html
        // Note that wind here is in World frame

        const Vector3r wind = VectorMath::transformToBodyFrame(wind_world, orientation);

        // Utils::log(Utils::stringf("Wind in body frame: %s, world frame: %s", VectorMath::toString(wind).c_str(),
        //                             VectorMath::toString(wind_world).c_str()));

        Wrench wrench = Wrench::zero();
        const real_T air_density = body.getEnvironment().getState().air_density;

        // Body is approximated as a box, with six faces (0-5)
        // Wind force on opposite faces cancel out, therefore we take only one of each pair
        // 0 -> top, 2 -> left, 4 -> back, this depends on how drag faces are filled, see MultiRotorPhysicsBody->createDragVertices() (L174)
        // https://github.com/microsoft/AirSim/blob/master/AirLib/include/vehicles/multirotor/MultiRotorPhysicsBody.hpp#L195
        // Hacky solution, better way would be find out the faces which would face the wind, and use those
        for (uint vi = 0; vi < body.dragVertexCount(); vi+=2) {
            const auto& vertex = body.getDragVertex(vi);
            // Here normal is assumed to be unit vector
            const Vector3r& drag_face_normal = vertex.getNormal();

            // Force (normal to face) = Pressure x Area
            // Velocity of Wind perpendicular to face
            const real_T wind_vel_comp = drag_face_normal.dot(wind);
            // Pressure along the normal, retain the sign of the velocity component
            const real_T pressure = 0.5f * air_density * wind_vel_comp * abs(wind_vel_comp);

            // Force along the normal, the sign from pressue indicates the direction of force, whether opposite or not
            const Vector3r wind_force = drag_face_normal * (vertex.getDragFactor() * pressure);
            const Vector3r wind_torque = vertex.getPosition().cross(wind_force);

            // Utils::log(Utils::stringf("Wind force: %s, Wind Torque: %s", VectorMath::toString(wind_force).c_str(),
            //                             VectorMath::toString(wind_torque).c_str() ));

            wrench.force += wind_force;
            wrench.torque += wind_torque;
        }

        // Utils::log(Utils::stringf("Total Wind force: %s, Total Wind torque: %s", VectorMath::toString(wrench.force).c_str(),
        //                             VectorMath::toString(wrench.torque).c_str()));
        ...
        Kinematics::State& next, Wrench& next_wrench, const Vector3r& wind, const Vector3r& ext_force)
    {
        ...
        if (body.isGrounded()) {
            ...
            const Wrench wind_wrench = getWindWrench(body, current.pose.orientation, wind);
            const Wrench ext_force_wrench = getExtForceWrench(body, current.pose.orientation, ext_force);

            next_wrench = body_wrench + drag_wrench + wind_wrench + ext_force_wrench;
